 ...
 */
export const getStartYs = (data, margin) => data
 ...
 */
export const getMerges = (data, index) => {
 ...
 */
export const getHistories = (data, index) => {
    const [, replicas] = data[index];
 ...
 */
export const getStateCoordinates = (data, index, startY, margin) => {
    ...
    const stateCoordinates = replicas.map(({ id }) => ({
        replicaId: id,
        coordinates: [],
    }));
    ...
    while (!histories.every((history) => !history.length)) {
        histories.forEach((history, i) => {
            ...
            \ntimestamp = ${JSON.stringify(payload[1])}`,
                    ...
                    const { coordinates: fromCoordinates } = stateCoordinates.find(({ replicaId }) => replicaId === other_id);
 ...
 */
export const getLineCoordinates = (data, startYs, margin) => {
    const lineCoordinates = [];
    data.forEach((d, dataIndex) => {
        const stateCoordinates = getStateCoordinates(data, dataIndex, startYs[dataIndex], margin);
 ...
 */
export const getSymbolCoordinates = (data, startYs, margin) => getLineCoordinates(data, startYs, margin).map(({ x_2, y_2, replicaId, title }) => ({
    x: x_2,
    y: y_2,
    replicaId,
    title,
 ...
 */
export const getCircleCoordinates = (data, startYs, margin) => {
 ...
 */
export const getTimelineCoordinates = (data, startYs, margin) => {
